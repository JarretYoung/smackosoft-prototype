...
-- ACCOUNTS
-- ============================================================
-- * only accounts references auth.users; everything else scopes
--   created_by/updated_by to accounts(id), which is never deleted
-- * no created_by/updated_by here: only the row's owner can ever
--   write to it, so both would always just be itself

-- schema ------------------------------------------------------

CREATE FUNCTION public.set_accounts_audit_fields()
RETURNS trigger
LANGUAGE plpgsql
SET search_path = ''
AS $$
...
-- case-insensitive uniqueness; NULL handles never collide
...
CREATE TRIGGER "accounts_set_audit_fields"
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.set_accounts_audit_fields();
...
-- SESSIONS & ACCESS
-- ============================================================
-- * merged: session_access gates sessions, but sessions' policies
--   need has_session_access, which needs session_access to exist first
...
-- forces these fields from the server, ignoring whatever the client sends
...
CREATE TABLE "public"."session_access" (
  ...
  CONSTRAINT "session_access_updated_by_fkey" FOREIGN KEY (updated_by) REFERENCES public.accounts(id)
);
...
CREATE TRIGGER "session_access_set_audit_fields"
  BEFORE INSERT ON "public"."session_access"
  FOR EACH ROW
  EXECUTE FUNCTION public.set_audit_fields();

-- access control ----------------------------------------------

-- without this, a fresh session would have no accessor at all
CREATE FUNCTION public.grant_creator_session_access()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
AS $$
BEGIN
  INSERT INTO public.session_access (session_id, account_id)
  VALUES (NEW.id, public.current_account_id());
  RETURN NEW;
END;
$$;

CREATE TRIGGER "sessions_grant_creator_access"
  AFTER INSERT ON "public"."sessions"
  FOR EACH ROW
  EXECUTE FUNCTION public.grant_creator_session_access();

-- no re-grant: only the trigger manager ever invokes this, and that
-- doesn't require EXECUTE on the role that fired the trigger
REVOKE ALL ON FUNCTION public.grant_creator_session_access() FROM PUBLIC;

-- used to protect the creator's own access grant, and to gate session deletion
...
-- unconditional: access is granted by a trigger after the row exists
...
-- creator-only, not accessor-based like everything else: deleting a
-- session is one-way and irreversible for its history, unlike edits
...
-- excludes the creator's own row, so it can never be revoked
...
CREATE TRIGGER "matches_set_audit_fields"
  BEFORE INSERT OR UPDATE ON "public"."matches"
  FOR EACH ROW
  EXECUTE FUNCTION public.set_audit_fields();

-- used by every table below this one
...
CREATE TRIGGER "match_teams_set_audit_fields"
  BEFORE INSERT OR UPDATE ON "public"."match_teams"
  FOR EACH ROW
  EXECUTE FUNCTION public.set_audit_fields();

-- used by match_players and match_game_scores
...
CREATE TRIGGER "match_games_set_audit_fields"
...
CREATE TRIGGER "match_players_set_audit_fields"
...
CREATE TRIGGER "match_game_scores_set_audit_fields"
  BEFORE INSERT OR UPDATE ON "public"."match_game_scores"
  FOR EACH ROW
  EXECUTE FUNCTION public.set_audit_fields();
